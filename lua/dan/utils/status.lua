--- ### dan Status API
--
-- Statusline related utility functions to use within dan and user configurations.
--
-- This module can be loaded with `local status = require "dan.utils.status"`
--
-- @module dan.utils.status
-- @copyright 2022
...
M.env.separators = dan.user_opts("heirline.separators", {
  none = { "", "" },
  left = { "", "  " },
  right = { "  ", "" },
  center = { "  ", "  " },
...
M.env.attributes = dan.user_opts("heirline.attributes", {
...
M.env.icon_highlights = dan.user_opts("heirline.icon_highlights", {
  file_icon = {
...
M.env.sign_handlers = dan.user_opts("heirline.sign_handlers", M.env.sign_handlers)
...
function M.hl.lualine_mode(mode, fallback)
  local lualine_avail, lualine = pcall(require, "lualine.themes." .. (vim.g.colors_name or "dan"))
...
-- @usage local heirline_component = { provider = "Example Provider", hl = require("dan.utils.status").hl.mode },
...
-- @usage local heirline_component = require("heirline.utils").surround({ "|", "|" }, require("dan.utils.status").hl.mode_bg, heirline_component),
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.fileicon(), hl = require("dan.utils.status").hl.filetype_color },
...
-- @usage local heirline_component = { provider = "Example Provider", hl = require("dan.utils.status").hl.get_attributes("treesitter") },
...
-- @usage local heirline_component = { provider = "Example Provider", hl = require("dan.utils.status").hl.file_icon("winbar") },
...
-- @usage local heirline_component = { init = require("dan.utils.status").init.breadcrumbs { padding = { left = 1 } } }
function M.init.breadcrumbs(opts)
  ...
  return function(self)
    ...
    if opts.max_depth and opts.max_depth > 0 then
      ...
      if start_idx > 0 then
        table.insert(children, { provider = require("dan.utils").get_icon "Ellipsis" .. opts.separator })
      end
    end
...
-- @return The Heirline init function
-- @usage local heirline_component = { init = require("dan.utils.status").init.separated_path { padding = { left = 1 } } }
...
-- @return The Heirline init function
-- @usage local heirline_component = { init = require("dan.utils.status").init.update_events { "BufEnter", { "User", pattern = "LspProgressUpdate" } } }
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.fill }
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.signcolumn }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.numbercolumn }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.foldcolumn }
-- @see dan.utils.status.utils.stylize
function M.provider.foldcolumn(opts)
  opts = extend_tbl({ escape = false }, opts)
  local ffi = require "dan.utils.ffi" -- get dan C extensions
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.tabnr() }
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.spell() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.paste() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.macro_recording() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.search_count() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.mode_text() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.percentage() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.ruler({ pad_ruler = { line = 3, char = 2 } }) }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.scrollbar() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.close_button() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.filetype() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.filename() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.unique_path() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.file_modified() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.file_read_only() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.file_icon() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.git_branch() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.git_diff({ type = "added" }) }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.diagnostics({ severity = "ERROR" }) }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.lsp_progress() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.lsp_client_names({ expand_null_ls = true, truncate = 0.25 }) }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.treesitter_status() }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("dan.utils.status").provider.str({ str = "Hello" }) }
-- @see dan.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.is_active }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = function() return require("dan.utils.status").condition.buffer_matches { buftype = { "terminal" } } end }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.is_macro_recording }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.is_hlsearch }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.is_git_repo }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.git_changed }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.file_modified }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.file_read_only }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.has_diagnostics }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.has_filetype }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.aerial_available }
...
-- @param bufnr a buffer number to check the condition for, a table with bufnr property, or nil to get the current buffer
-- @return boolean of whether or not LSP is attached
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.lsp_attached }
...
-- @usage local heirline_component = { provider = "Example Provider", condition = require("dan.utils.status").condition.treesitter_available }
...
-- @usage local string = require("dan.utils.status").utils.stylize("Hello", { padding = { left = 1, right = 1 }, icon = { kind = "String" } })
...
-- @usage local heirline_component = require("dan.utils.status").component.fill()
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.file_info()
...
-- @param opts options for configuring file_icon, filename, filetype, file_modified, file_read_only, and the overall padding
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.tabline_file_info()
function M.component.tabline_file_info(opts)
  return M.component.file_info(extend_tbl({
    ...
    close_button = {
      ...
      on_click = {
        callback = function(_, minwid) require("dan.utils.buffer").close(minwid) end,
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.nav()
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.cmd_info()
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.mode { mode_text = true }
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.breadcumbs()
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.separated_path()
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.git_branch()
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.git_diff()
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.diagnostics()
...
-- @param opts options for configuring diagnostic providers and the overall padding
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.treesitter()
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.lsp()
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.foldcolumn()
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.numbercolumn()
...
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").component.signcolumn()
...
--- A general function to build a section of dan status providers with highlights, conditions, and section surrounding
-- @param opts a list of components to build into a section
-- @return The Heirline component table
-- @usage local heirline_component = require("dan.utils.status").components.builder({ { provider = "file_icon", opts = { padding = { right = 1 } } }, { provider = "filename" } })
...
-- @usage local heirline_component = { on_click = { callback = function(...) local args = require("dan.utils.status").utils.statuscolumn_clickargs(...) end } }
...
M.heirline.make_buflist = function(component)
  ...
  return require("heirline.utils").make_buflist(
    ...
    function() return vim.t.bufs end, -- use dan bufs variable
